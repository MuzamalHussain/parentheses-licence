...
import { useAdminOrders, useAdminOrderStats, useMarkRefunded } from "../../hooks/useOrders";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl w-full max-w-sm shadow-2xl p-6 space-y-4">
        ...
        <p className="text-sm text-gray-500">
          This records the refund and <span className="font-medium text-gray-700">revokes the associated license</span> immediately.
          Actually processing the refund through {order.gateway === "stripe" ? "Stripe" : "the local gateway"} must be done separately for now.
        </p>
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <StatCard label="Paid" value={stats?.stats.paid} icon={ShoppingCart} color="text-brand-600 bg-brand-50" />
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {["", "stripe", "local"].map((g) => (
            <button key={g} onClick={() => { setGateway(g); setPage(1); }}
              ...
              {g === "" ? "All gateways" : g === "stripe" ? "Stripe" : "Local"}
            </button>
          ))}
        </div>
