...
import { Package, Check, Loader2, CreditCard, Smartphone, Tag, X } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl w-full max-w-md shadow-2xl">
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`flex flex-col items-center gap-2 p-4 rounded-lg border-2 transition-colors ${
                  gateway === "stripe" ? "border-brand-500 bg-brand-50" : "border-gray-200 hover:border-gray-300"
                }`}
              >
                <CreditCard className={`w-5 h-5 ${gateway === "stripe" ? "text-brand-600" : "text-gray-400"}`} />
              ...
              <button
                ...
                className={`flex flex-col items-center gap-2 p-4 rounded-lg border-2 transition-colors ${
                  gateway === "local" ? "border-brand-500 bg-brand-50" : "border-gray-200 hover:border-gray-300"
                }`}
              >
                <Smartphone className={`w-5 h-5 ${gateway === "local" ? "text-brand-600" : "text-gray-400"}`} />
          ...
          <Button onClick={handlePay} loading={isPending} className="w-full">
            Continue to payment
          </Button>
          <p className="text-xs text-gray-400 text-center">
            You'll be redirected to a secure {gateway === "stripe" ? "Stripe" : "payment"} page to complete your purchase.
          </p>
        </div>
      </div>
    </div>
