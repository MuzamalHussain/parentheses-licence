...
router.get("/tickets/:id",           validateRequest({ params: idParamSchema }), c.getTicket);
router.post("/tickets/:id/reply",    validateRequest({ params: idParamSchema }), validate(replySchema), c.replyToTicket);
router.patch("/tickets/:id/status",  validateRequest({ params: idParamSchema }), validate(statusSchema), c.updateStatus);
